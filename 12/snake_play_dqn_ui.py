"""
snake_play_dqn_ui.py
功能：載入 DQN 訓練模型，自動玩 Snake，帶 UI 顯示
...
# ----------------------------
class SnakeGame:
    def __init__(self, width=400, height=400, block_size=20, render=True):
        self.width = width
        self.height = height
        ...
        if self.render:
            pygame.init()
            self.screen = pygame.display.set_mode((width, height + 40))  # 多出 40 px 顯示分數/動作
            pygame.display.set_caption("DQN Snake UI Play")
    ...
    def render_game(self, action_idx):
        ...
        # 畫分數與動作
        action_text = ['Left', 'Straight', 'Right'][action_idx]
        score_surface = self.font.render(f"Score: {self.score} | Action: {action_text}", True, (255,255,255))
        self.screen.blit(score_surface, (10, self.height + 10))
        pygame.display.flip()
        self.clock.tick(10)
...
# 主程式
# ----------------------------
def play_ui(model_path="dqn_snake.pth"):
